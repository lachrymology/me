;;; ix-mode.el --- Broccoli editing mode.
...
(defgroup ix nil 
  "Customization variables for `ix-mode'."
  :tag "Ix"
  :group 'languages)

(defcustom ix-indent-level 4
  "Number of spaces for each indentation step."
  :type 'integer
  :group 'ix)

(defcustom ix-auto-indent-flag t
...
(defvar ix-mode-map nil 
  "Keymap used in Ix mode.")

(unless ix-mode-map 
  (setq ix-mode-map (make-sparse-keymap)))

(when ix-auto-indent-flag
  (mapc (lambda (key) 
	  (define-key ix-mode-map key 'ix-insert-and-indent))
	'("{" "}" "(" ")" ":" ";" ",")))

(defun ix-insert-and-indent (key)
...
(defconst js-keyword-re
  ...
  "Regular expression matching any Ix keyword.")

(defconst js-basic-type-re
  (regexp-opt '("boolean" "byte" "char" "double" "float" "int" "long"
                "short" "void") 'words)
  "Regular expression matching any predefined type in Ix.")

(defconst js-constant-re
  (regexp-opt '("false" "null" "true") 'words)
  "Regular expression matching any future reserved words in Ix.")
  ...
  (save-excursion
    ...
    (when (save-excursion
      ...
      (save-excursion
        ...
        (+ (current-indentation) ix-indent-level)))))
             ...
             (if (looking-at "[({[][ \t]*\\(/[/*]\\|$\\)")
                 (progn
                   ...
                   (cond (same-indent-p
                          (current-column))
                         (continued-expr-p
                          (+ (current-column) (* 2 ix-indent-level)))
                         (t
                          (+ (current-column) ix-indent-level))))
               (unless same-indent-p
                 (forward-char)
                 (skip-chars-forward " \t"))
               (current-column)))
	    (continued-expr-p ix-indent-level)
            (t 0)))))


(defun ix-indent-line ()
  "Indent the current line as Ix source text."
  (interactive)
  (let ((parse-status 
...
(defun ix-fill-paragraph (&optional justify)
...
;;;###autoload
(defun ix-mode ()
  "Major mode for editing Ix source text.
...
\\{ix-mode-map}"
  ...
  (use-local-map ix-mode-map)
  (set-syntax-table ix-mode-syntax-table)
  (set (make-local-variable 'indent-line-function) 'ix-indent-line)
